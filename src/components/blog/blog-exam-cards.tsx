      ...
      <div className="flex  items-start gap-4">
        <div className="flex flex-col gap-1">
          <div className="relative h-16 w-16">
            <Image src={item.logo_url} alt={item.name} width={64} height={64} />
            <div className="absolute bottom-0 right-0 bg-white p-1 rounded-full">
              <CourseCheckBadge size={20} fill="#0083ff" />
            </div>
          </div>
          <div className="text-center">
            <h3 className="heading-medium !font-semibold text-black">
              {item.short_name}
            </h3>
          ...
          {points.map((item, index) => {
            ...
            return (
              <div key={index} className="flex items-center gap-1">
                ...
                <span className="body-medium !font-normal surface-text-gray-muted">
                  {item?.name}:
                </span>
