...
const PAYLOAD_API = `${process.env.BACKEND_URL}`;
...
export async function GET(
  ...
  if (!allowedCourses.includes(originalExamName)) {
    return new NextResponse(`${originalExamName} Exam not Allowed`, {
      status: 404,
    });
  }
  ...
  try {
    ...
    if (!exam) {
      return new NextResponse(`${originalExamName} Exam not found`, {
        status: 404,
      });
    }
    ...
    const levelsRes = await fetch(
      `${PAYLOAD_API}/blog/get-enavigation?${queryLevels}`,
    );
    ...
    for (const level of levels) {
      ...
      const subjectsRes = await fetch(
        `${PAYLOAD_API}/blog/get-subject?${querySubjects}`,
      );
      ...
      const yearsRes = await fetch(
        `${PAYLOAD_API}/blog/get-years?${queryYears}`,
      );
    ...
    console.error(error);
    return new NextResponse(`Error generating sitemap for ${error}`, {
      status: 500,
    });
  }
}
